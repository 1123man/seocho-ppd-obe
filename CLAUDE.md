...
- **ViewModel**: `StateFlow` + immutable data class로 UI 상태 관리. Intent → ViewModel → State → UI 흐름.
- **Composable**: `collectAsStateWithLifecycle` 사용, state hoisting (상태는 받고 변경은 콜백으로 위임).
- **Navigation**: 단일 `NavHost`, 개별 화면은 `NavController`를 직접 참조하지 않고 상위에서 주입된 콜백만 사용.
- **DI**: ViewModel/Repository/DataSource 모두 DI 컨테이너에서 관리, UI에서 직접 생성 금지.
